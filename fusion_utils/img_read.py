...
def img_read(path, mode):
    ...
    if mode == 'RGB' or mode == 'L':
        ...
        img = np.asarray(Image.open(path).convert('RGB'), dtype=np.float32) #np, rgb
        
        img = image_to_tensor(img, keepdim=True) / 255.0 #np -> tensor
        #print("before convert img shape", img.shape)
        img = rgb_to_ycbcr(img)
        #print("after convert img shape", img.shape)
