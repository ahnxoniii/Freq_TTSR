...
def resize_fre(HR, size):
    # [-1, 1] 범위를 [0, 255] 범위로 변환
    HR_255 = ((HR + 1) * 127.5).clip(0, 255).astype(np.uint8)
    
    # NumPy 배열을 PIL 이미지로 변환
    hr_pil = Image.fromarray(HR_255)
    # 크기 조절
    hr_resized_pil = hr_pil.resize(size, Image.BICUBIC)
    
    # 다시 NumPy 배열로 변환
    hr_resized = np.array(hr_resized_pil).astype(np.float32)
    
    # [0, 255] → [-1, 1] 범위로 변환
    hr_resized = (hr_resized / 127.5) - 1
    return hr_resized
    ...
    def train(self, current_epoch=0, is_init=False):
        ...
        for i_batch, sample_batched in enumerate(self.dataloader['train']):
            ...
            if (not is_init):
                ...
                if ('fre_loss' in self.loss_all):
                    fre_loss = self.args.fre_w * self.loss_all['fre_loss'](sr, hr)
                    loss += fre_loss
                    if (is_print):
                        self.logger.info( 'fre_loss: %.10f' %(fre_loss.item()) )
